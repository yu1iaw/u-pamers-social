...
export const MessageBubble = ({type, message}) => {
    ...
    const dateFormat = new Date(message.createdAt).toString().slice(-18, -12);

    return (
        <View style={tw`p-2 ${type === "myOwn" ? 'self-end gap-y-1' : type === "system" ? 'self-center' : 'self-start'}`}>
            <View style={tw`flex-row gap-x-3 max-w-[60%]`}>
                {type === "notMine" && <Image source={{uri: message.image ?? 'https://shorturl.at/dADKQ'}} style={tw`w-[30px] h-[30px] rounded-full self-end`} />}
                <View style={tw`p-2 rounded-xl shadow ${bubbleStyle}`}>
                    <Text style={tw.style(`text-sm ${textStyle}`, { fontFamily: "i_medium" })}>{message.text}</Text>
                </View>
